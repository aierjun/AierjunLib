 ...
 */
public abstract class UploadCallBackUtils {
     ...
     * @author ylbf_dev
     */
    public static int uploadFile(final ArrayList<File> imagefiles,final ArrayList<File> videofiles, final String uploadUrl,final ArrayList<String> name,final ArrayList<Object> values, UploadFileCallBack uploadFileCallBack) {
        try {
            ...
            if (videofiles != null)
                addVedio(videofiles, dos,uploadFileCallBack);
            dos.flush();
            ...
            if (uploadFileCallBack != null)
                uploadFileCallBack.onCallBackCodeMes(state,mes);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return state;
    ...
    private static void addVedio(ArrayList<File> files, DataOutputStream output,UploadFileCallBack uploadFileCallBack) {
        if (files.size()>0&&files!=null){
            try {
                StringBuilder sb = new StringBuilder();
                sb.append(PREFIX + BOUNDARY + LINE_END);
                ...
                while ((len = is.read(bytes)) != -1) {
                    output.write(bytes, 0, len);
                    if (uploadFileCallBack != null)
                        uploadFileCallBack.onProgress(output.size() * 1.0f / (files.get(0).length() + headerSize));
                }
                is.close();
                output.write(LINE_END.getBytes());
